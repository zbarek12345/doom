...
Player::Player(NewModels::vec3 position, float angle) {
	...
	this->pos_y = static_cast<float>(position.y);
	this->pos_z = static_cast<float>(position.z);
...
void Player::HandleEvent(SDL_Event* event, double deltaTime) {
	...
	float speed = 200.0f * deltaTime; // Adjusted for Doom map scale

	float yaw_rad = camera->GetYaw() * M_PI / 180.0f;
	float pitch_rad = camera->GetPitch() * M_PI / 180.0f;
	float forward_x = cosf(yaw_rad) * cosf(pitch_rad);
	float forward_y = sinf(pitch_rad);
	float forward_z = sinf(yaw_rad) * cosf(pitch_rad);
	...
	pos_x += forward_x * move_forward + right_x * move_strafe;
	pos_z += forward_y * move_forward + move_vertical;
	pos_y += forward_z * move_forward + right_z * move_strafe;
...
void Player::Render() {
	camera->Render();
	glTranslatef(-pos_x, -pos_z, pos_y);
}